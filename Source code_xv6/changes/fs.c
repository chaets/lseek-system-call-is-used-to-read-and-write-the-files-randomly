...
#define FIRST_THREE_BYTES(p)  (p>>8)                                                    
#define LAST_EIGHT_BITS(p) (p & 0xFF) 
#define COMBINE_ADD_TO_LEN(p,k)   ((p<<8)+(0xFF & k))   
  ...
  if(ip->type==T_EXTENT){  // checking type of the file
    ...
    for(k=0;k<(NDIRECT/2);k++){ // run through the length of the pointers here ndirect represent 12
      if(ip->addrs[k]==0) // if the address is zero
          break;
      if((ip->addrs[k+NDIRECT/2]+LAST_EIGHT_BITS(ip->addrs[k]))>bn && ip->addrs[k+NDIRECT/2]<=bn ) // this condition corresponds to the neighbouring block check.
   ...
   // the below condition is useful to add new extents to the given file.
    ...
    ip->addrs[k]=COMBINE_ADD_TO_LEN(blkadd, 1); //address of 32bit combining first three bytes of block address and 1 byte of length as address.
    ip->addrs[k+NDIRECT/2]=bn;
  }
   return blkadd; // return the address corresponding to the block.

  }// end of the extent type file conditions.
  else{
  if(bn < NDIRECT){
...
itrunc(struct inode *ip)
{
  ...
  struct buf *bp;
  uint *a;
  if(ip->type==T_EXTENT){ //file check for extent type.
    for(i=0;i<NDIRECT/2;i++){ // run through the address pointers of the inode.
      for(j=0;j<LAST_EIGHT_BITS(ip->addrs[i]);j++){ //run through the last eight bits of the inode address.
          bfree(ip->dev,FIRST_THREE_BYTES(ip->addrs[i])+j*BSIZE); //freeing the memory.
      }
    ip->addrs[i]=0; // setting address of the inode to zero.
    ip->addrs[i+NDIRECT/2]=0;
    }
  }else{
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
...
stati(struct inode *ip, struct stat *st)
{
  ...
  if(ip->type==T_EXTENT){ 
  int i;
  cprintf("Details of extent based file:");
  for(i=0;i<NDIRECT/2;i++){
    if(ip->addrs[i]!=0)
     {
	uint addr = ip->addrs[i] >> 8;
	uint offset = ip->addrs[i+1];
	uint length = ip->addrs[i] & 0xFF;
	cprintf("\nbase addr: %x\toffset: %d\tlength: %d\t",
	 addr,
	 offset,
	 length);
   }
